...
contract HybridDisputeBoard is SigVerifier {
    ...
    function initializeChannel(bytes32 _foreignId, address[] calldata _parties) external {
        ...
        channels[_internalId] = ChannelInstance(Status.INITIALIZED, 0, _parties, 0, 0);
    ...
    function dispute(bytes32 _id, uint _version, bytes32 _stateHash, bytes calldata _signatures) external {
        ...
        require(_instance.status == Status.INITIALIZED ||(_instance.status == Status.SUBMITTED && (_instance.version < _version && _instance.timeout > now)),"Wrong version submitted");
        
        //Check signatures
        verifySignatures(_instance.parties, abi.encode(address(this), _id, _version, _stateHash), _signatures);
        
        //Set timeout, status and emit event if this is a new dispute (new dispute means status INITIALIZED or higher lockCount)
        if(_instance.status == Status.INITIALIZED){
    ...
    function getChannelInstance(bytes32 _id) external view returns (uint, bytes32, bool){
        ...
        return (channels[_id].version,  channels[_id].stateHash, _unlockable);
    }
} 
