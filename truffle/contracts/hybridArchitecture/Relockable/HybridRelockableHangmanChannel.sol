...
contract HybridRelockableHangmanChannel is AbstractHangmanChannel, SigVerifier{
    ...
    function lock(bytes calldata _signatures) external isChannelStatus(ChannelStatus.ONCHAIN){
        
        //Check signatures
        //!!! Signatures need to include the contracts address as well
        verifySignatures(parties, abi.encode(address(this),lockCount), _signatures);
    ...
    function unlock(bytes32 _hashedWord, bool[26] calldata _guessed, bytes calldata _word, uint8 _currentGuess, uint _wordLength, uint _missingLetters, uint _tries, Stage _stage) external isChannelStatus(ChannelStatus.OFFCHAIN){
        ...
        (uint _unlockCount, ,bytes32 _stateHash, bool _unlockable) = db.getChannelInstance(keccak256(abi.encode(address(this), 0)));
        
        //Check that this state has not been unlocked before
        require(_unlockCount == lockCount, "State has already been unlocked");
    ...
    //****************
    //Getters
    function getState() external view returns (ChannelStatus, uint, address[] memory, bytes32, bool[26] memory, bytes memory, uint8, uint, uint, uint, Stage){
        return (status, lockCount, parties, hashedWord, guessed, word, currentGuess, wordLength, missingLetters, tries, stage);
    }
}
